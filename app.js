...
const settingsFontDown = $("#settingsFontDown");
const settingsFontUp = $("#settingsFontUp");
const fontSizeValue = $("#fontSizeValue");
const lineHeightDown = $("#lineHeightDown");
const lineHeightUp = $("#lineHeightUp");
const lineHeightValue = $("#lineHeightValue");
const marginDown = $("#marginDown");
const marginUp = $("#marginUp");
const marginValue = $("#marginValue");
const mobilePrev = $("#mobilePrev");
const mobileNext = $("#mobileNext");
const mobileBookmark = $("#mobileBookmark");
...
content?.addEventListener("click", (event) => {
  if (!activeBook() || !window.matchMedia("(max-width: 900px)").matches) return;
  if (window.getSelection()?.toString()) return;
  const rect = content.getBoundingClientRect();
  const x = event.clientX - rect.left;
  if (x < rect.width * 0.23) openAdjacentChapter(-1);
  if (x > rect.width * 0.77) openAdjacentChapter(1);
});

chapterSelect?.addEventListener("change", () => openChapter(Number(chapterSelect.value)));
prevChapter?.addEventListener("click", () => openAdjacentChapter(-1));
nextChapter?.addEventListener("click", () => openAdjacentChapter(1));
...
settingsFontDown?.addEventListener("click", () => changeFont(-1));
settingsFontUp?.addEventListener("click", () => changeFont(1));
lineHeightDown?.addEventListener("click", () => changeLineHeight(-0.05));
lineHeightUp?.addEventListener("click", () => changeLineHeight(0.05));
marginDown?.addEventListener("click", () => changeMargin(-1));
marginUp?.addEventListener("click", () => changeMargin(1));
settingsButton?.addEventListener("click", toggleSettingsPanel);
mobileSettings?.addEventListener("click", toggleSettingsPanel);
mobilePrev?.addEventListener("click", () => openAdjacentChapter(-1));
mobileNext?.addEventListener("click", () => openAdjacentChapter(1));
mobileBookmark?.addEventListener("click", saveBookmark);
...
function renderChapterControls(book) {
  ...
  if (mobilePrev) mobilePrev.disabled = index <= 0;
  if (mobileNext) mobileNext.disabled = index >= chapters.length - 1;
}

function openAdjacentChapter(delta) {
  const book = activeBook();
  if (!book) return;
  openChapter(currentChapterIndex(book) + delta);
}
...
function buildDemoBook() {
  ...
  const chapterTexts = [
    ...
    [
      "????? 4 - ????????????????",
      "???????????????? ???? export book ???????? backup ??????????? LINE ????????? ???????????????????????? ???????? Restore book backup ?????????????????????????? shelf",
      "?????????????????????? cloud ???? ? ????????? browser storage ???????????????????????? ???????????? ?????????????? backup ????????????",
      "?????????????????????????????? ?????????????????? ????? ?????????????????????"
    ]
  ];
...
function setButtonsDisabled(disabled) {
  [bookmarkButton, jumpBookmarkButton, exportButton, exportSidebarButton, mobileBookmark, mobilePrev, mobileNext].forEach((button) => {
...
function applySettings() {
  ...
  document.documentElement.style.setProperty("--reader-padding", `${settings.margin}px`);
  setText(themeButton, settings.dark ? "Day" : "Night");
  setText(fontSizeValue, `${settings.fontSize}px`);
  setText(lineHeightValue, settings.lineHeight.toFixed(2));
  setText(marginValue, marginLabel(settings.margin));
}

function changeFont(delta) {
  settings.fontSize = Math.max(17, Math.min(34, settings.fontSize + delta));
  applySettings();
  persist(false);
}

function changeLineHeight(delta) {
  settings.lineHeight = Math.max(1.55, Math.min(2.35, Math.round((settings.lineHeight + delta) * 100) / 100));
  applySettings();
  persist(false);
}

function changeMargin(delta) {
  const options = [14, 18, 22, 28, 34];
  const current = options.reduce((best, value, index) => Math.abs(value - settings.margin) < Math.abs(options[best] - settings.margin) ? index : best, 0);
  settings.margin = options[Math.max(0, Math.min(options.length - 1, current + delta))];
  applySettings();
  persist(false);
}
...
function marginLabel(value) {
  if (value <= 14) return "Tight";
  if (value >= 34) return "Wide";
  return "Normal";
}
...
function loadSettings() {
  try {
    return { fontSize: 21, lineHeight: 1.9, margin: 22, dark: false, ...JSON.parse(localStorage.getItem(settingsKey)) };
  } catch {
    return { fontSize: 21, lineHeight: 1.9, margin: 22, dark: false };
  }
}
